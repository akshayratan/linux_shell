...
struct job_t *job= NULL; 
     if( argv[1] != NULL && argv[1][0] == '%' ){     // job id
        int jid = atoi( &argv[1][1] );
        if( !( job = getjobjid( jobs, jid ) ) ){
            printf( "%s no such job found\n", argv[1] );
            return;
        }
    } else if( argv[1] != NULL && isdigit(argv[1][0]) ){       // pid
...
if ( WIFSIGNALED( status ) ){
        ...
        // check if the process had a clean exit
     ...
     printf("signal that stopped the job [%d] (%d) is %d \n",pid2jid(pd),pd,WSTOPSIG(status));
      }
   }    
     return;
}
